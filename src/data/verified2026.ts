// Mirror de src/data/verified2026.mjs para consumo desde React/TS.
// Si editas uno, edita el otro (mismos valores verificados el 28-08-2026).
...
export type VerifiedPrice = {
  plan: string | null;
  annualNetCLP: number | null;
  tax: 'neto' | 'con IVA' | 'USD' | null;
  category: 'shared' | 'vps' | 'cloud';
  label: string;
  detail: string;
};

export const VERIFIED_PRICING: Record<string, VerifiedPrice> = {
  hostingplus: {
    plan: 'Personal NVME',
    annualNetCLP: 64900,
    tax: 'neto',
...
export function getVerifiedPricing(slug?: string | null): VerifiedPrice | null {
  if (!slug) return null;
  const key = PRICING_ALIASES[slug] || slug;
  return VERIFIED_PRICING[key] || null;
}

export const PIT_CHILE_MEMBERS: Record<string, { ip: string; speed: string; note: string }> = {
...
export function hasPitChile(asn?: string | null): boolean {
  if (!asn) return false;
  return Object.prototype.hasOwnProperty.call(
    PIT_CHILE_MEMBERS,
    String(asn).toUpperCase().replace(/\s/g, ''),
  );
}

export function pitChileLabel(asn?: string | null): string {
  const m = PIT_CHILE_MEMBERS[String(asn || '').toUpperCase()];
  if (!m) return 'No';
  return `Sí · ${m.ip} · ${m.speed}`;
}

export const DATACENTER_FACTS: Record<string, { label: string; tenancy: 'propio' | 'colocation'; note: string }> = {
  hostingplus: {
...
export const CORPORATE_FACTS: Record<string, { legal: string | null; rut: string | null; note: string }> = {
  bluehosting: {
  ...
  hostname: { legal: 'Servicios Informáticos Hostname Limitada', rut: '76.096.415-8', note: '' },
  hostingcl: {
    legal: null,
    rut: null,
...
export const BGP_FACTS: Record<string, { prefixesV4: string; metric: string; note: string }> = {
  AS263237: {
