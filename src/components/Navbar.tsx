...
const Navbar = () => {
  ...
  return <nav className="container mx-auto px-4 flex justify-between items-center h-16">
      ...
      <button onClick={toggleMobileMenu} className="md:hidden text-[#2B2D42] focus:outline-none">
        {isMobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
      </button>
      
      <div className={`md:hidden fixed top-0 left-0 w-full h-full bg-white transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-50`}>
