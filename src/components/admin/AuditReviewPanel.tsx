...
// Fields we can extract from scraped_data and map to hosting_companies columns
...
function extractMappedFields(scraped: any): Record<string, any> {
  ...
  for (const [scraperKey, dbCol] of Object.entries(MAPPABLE_FIELDS)) {
    if (scraped[scraperKey] != null && scraped[scraperKey] !== '') {
      // For description, prefer description_seo over mission_statement
...
export default function AuditReviewPanel() {
  ...
  // Fetch company data for all pending audits
  ...
  const approveAudit = useMutation({
    mutationFn: async ({ auditId, fields }: { auditId: string; fields?: Record<string, any> }) => {
      ...
      if (Object.keys(mapped).length === 0) {
        throw new Error('No hay campos para aplicar');
      }

      // Apply to hosting_companies
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['audit-logs-pending'] });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Auditorías Pendientes ({audits.length})</h3>
      </div>
          ...
          <Card key={audit.id} className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Scrapeado: {new Date(audit.created_at).toLocaleString('es-CL')}
                    {complaints.severity && (
                      <Badge variant="outline" className="ml-2 text-[10px]">
                        Quejas: {complaints.severity}
                      </Badge>
                    )}
                    {scraped.pages_scraped && (
                      <span className="ml-2">{scraped.pages_scraped} páginas</span>
                    )}
                  </p>
                </div>
              </div>
              <div className="flex gap-2 items-center">
                ...
                <Button
                  size="sm"
                  onClick={() => approveAudit.mutate({ auditId: audit.id })}
                ...
                <Button
                  size="sm"
                  variant="destructive"
                  onClick={() => rejectAudit.mutate(audit.id)}
                  disabled={rejectAudit.isPending}
                >
              ...
              <div className="mt-4 space-y-3">
                          ...
                          <tr key={field} className={changed ? 'bg-yellow-500/5' : ''}>
                            <td className="p-2">
                              <Checkbox
                                ...
                                onCheckedChange={() => toggleField(audit.id, field)}
                              />
                            </td>
                ...
                <div className="flex justify-end">
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={selected.size === 0 || approveAudit.isPending}
                    onClick={() => handlePartialApprove(audit.id, scraped)}
                  >
                ...
                {complaints.results && complaints.results.length > 0 && (
                  <details className="text-xs">
                    <summary className="cursor-pointer text-muted-foreground">Ver quejas encontradas ({complaints.results.length})</summary>
                    <pre className="mt-1 p-2 bg-muted rounded overflow-auto max-h-32 text-[11px]">
                      {JSON.stringify(complaints.results, null, 2)}
                    </pre>
                  </details>
                )}
              </div>
